...
- Quantum Machines — https://www.quantum-machines.co/blog/ **[verified 2026-09-05 W36: PROMOTED from Discovered-source candidates — past the ≥2-run recurrence bar at times-seen 4+ (first seen 2026-08-31, re-confirmed 2026-09-01/02/05). CAVEAT: the individual blog-post URLs are JS-rendered behind a cookie-consent gate — `tvly extract` (both basic and `--extract-depth advanced`) returns only the consent/nav shell on every tested post, confirmed across 4 separate attempts (2026-08-31, 09-01, 09-02, 09-05); no working extraction method found yet for this domain specifically. Sweep as opened/degraded (nav-shell-only) each run; any claim from this source stays UNVERIFIED (queue-only, never evidence) until a working extraction path is found]** — control-software vendor (OPX controllers), publishes AI-for-quantum-hardware-calibration results
