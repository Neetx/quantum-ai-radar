...
- iopscience.org (IOP Publishing — Quantum Science and Technology journal) — times seen 3 — "QuSquare: scalable quality-oriented benchmark suite for pre-fault-tolerant quantum devices", DOI 10.1088/2058-9565/ae917c, Aguirre/Peña/Sanz — 3rd seen 2026-08-13 (discovered via a tracked Quantum Zeitgeist digest pointer — a digest path, not the non-digest path the W32 condition asked for, but the 3rd on-axis appearance from this domain either way; flagged for W33 to decide whether 3 appearances via any path now clears its own promotion bar) — 2nd seen 2026-08-12 (discovered via Quantum Frontiers' personal-essay blog post, a tracked independent-voice source but a NON-digest path — meets the W32-proposed 2nd-appearance promotion condition; "Integrability of Goldilocks quantum cellular automata", DOI 10.1088/2058-9565/ae57d5, published 2026-06); first appearance: "Quantum Adaptive Self-Attention for quantum Transformer models" (QASA), Quantum Sci. Technol. 11(3), DOI 10.1088/2058-9565/ae8b21, published 2026-07-29 — first seen 2026-08-05, discovered via a tracked Quantum Zeitgeist digest pointer; iopscience.org itself returns only a cookie-consent wall to `tvly extract`, publication metadata confirmed via Crossref API instead all three times) — FLAGGED for W33 promotion per the standing W32 proposal
...
- Sabine Hossenfelder — https://www.youtube.com/feeds/videos.xml?channel_id=UC1yNl2E66ZzKApQdRuTQ4tw **[channel_id resolved 2026-06-27; HEALED 2026-07-02 Pass 2: feed back to HTTP 200 after 404 for 2 runs; STRUCTURAL BREAK 2026-07-30: `/feeds/videos.xml` now 404s for EVERY tested channel incl. an unrelated control channel — a YouTube-side block on the endpoint, not a per-channel or proxy issue; still dead 2026-07-31. No working substitute found 2026-08-01 W31 either: `tvly search "<channel> latest video"` returns undated evergreen results (not usable as a recency signal, confirmed again this run), and `tvly extract` on the channel's `/videos` tab fails outright (JS-rendered, no fallback rendering). Log both Qiskit and Sabine as `degraded: youtube feeds.xml structurally dead, no substitute` until either the endpoint recovers or a working scrape method is found — this is a low-stakes INTAKE-only pointer lane (never evidence), so do not treat the absence as a coverage crisis, just keep logging it honestly]** — BROAD physics channel, but a prominent skeptic of quantum-computing hype; filter for quantum-computing / advantage-claim items only (serves the hype-skepticism mandate).
