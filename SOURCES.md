...
- quantum-machines.co (Quantum Machines) — times seen 2 — RL-agent two-qubit CZ-gate calibration with Academia Sinica, cutting calibration from ~15min to ~25s (reported Aug-30) — first seen 2026-08-31, re-checked 2026-09-01 (on-axis AI-for-quantum-hardware vendor; discovered via a Quantum Zeitgeist digest pointer. The vendor's own blog was 403'd 2026-08-31; re-attempted 2026-09-01 — no longer 403s, exact post URL now known (quantum-machines.co/blog/reinforcement-learning-cz-gate-calibration) but `tvly extract` only returns the cookie-consent/JS shell, not the article body. Now at 2 appearances across 2 runs, at the recurrence promotion bar — the weekly should attempt a different extraction method (e.g. WebFetch, or `--extract-depth advanced` on the direct post URL) before promoting, since the primary claim itself remains unverified)
