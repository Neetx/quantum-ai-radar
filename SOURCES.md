...
- quantum-machines.co (Quantum Machines) — times seen 3 — RL-agent two-qubit CZ-gate calibration with Academia Sinica, cutting calibration from ~15min to ~25s (reported Aug-30) — first seen 2026-08-31, re-checked 2026-09-01, 2026-09-02 (on-axis AI-for-quantum-hardware vendor; discovered via a Quantum Zeitgeist digest pointer. Now past the recurrence promotion bar (3 runs). Extraction still unresolved: `tvly extract` (both basic and `--extract-depth advanced`) on the exact post URL (quantum-machines.co/blog/reinforcement-learning-cz-gate-calibration) returns only the cookie-consent/JS shell; `WebFetch` on the same URL returns HTTP 403. The primary claim itself remains UNVERIFIED — the weekly should decide whether to promote the domain on recurrence alone (with the claim staying queued as unverified) or hold pending a working extraction method)
...
- **r/QuantumComputing** — read its Atom feed via the OLD-REDDIT domain: `curl -sL -A "Mozilla/5.0 (Windows NT 10.0; Win64; x64)" https://old.reddit.com/r/QuantumComputing/.rss` **[HEALED 2026-07-03 — the www.reddit.com `.rss` path (healed 2026-06-30 with a browser UA) started 403'ing again and stayed broken for 5 consecutive runs (Jun 30 Pass2 – Jul 3); www.reddit.com's `.json` API also 403s even with a UA. `old.reddit.com/.rss` with the same UA returns HTTP 200 with a full, current item list — confirmed working 2026-07-03. Use old.reddit.com going forward; retest www.reddit.com occasionally in case it un-blocks. WATCH 2026-09-01/RECURRED 2026-09-02: the login-wall mode recurred a 2nd time — now a forced-heal item. 2026-09-02 heal attempt: tried 2 alternate UAs (Googlebot UA → HTTP 403 "Blocked"; a generic curl UA on old.reddit.com → HTTP 403 with a Reddit anti-bot challenge page) and `tvly extract` on the same RSS URL (failed: "Failed to fetch url") and `WebFetch` on the subreddit page (refused: "unable to fetch from old.reddit.com") — all 4 alternates failed, no working substitute found this run. Escalating to the weekly for a different approach (e.g. a Reddit-mirror service, or dropping to a lower-frequency spot-check if no fix is found in 2-3 more runs) rather than re-trying the same exhausted methods daily]**. The canonical active quantum community.
