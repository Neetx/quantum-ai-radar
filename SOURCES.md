...
- arXiv **quant-ph** (core) — https://arxiv.org/list/quant-ph/recent **[verified 2026-06-26]** ; QML = `cat:quant-ph` cross-listed with `cs.LG`/`cs.ET` (metadata via `export.arxiv.org/api/query?search_query=cat:quant-ph+AND+abs:%22machine+learning%22&sortBy=submittedDate`). **DEGRADED 2026-09-14 (1st occurrence):** `export.arxiv.org/api/query` returned HTTP 429 "Rate exceeded" on every attempt (7 retries, exponential backoff 5s→120s) — a full outage of the query API this run, not a per-query issue. WORKING FALLBACK: `curl https://rss.arxiv.org/rss/quant-ph` (today's new-submissions listing, ~100 items) + `curl https://rss.arxiv.org/rss/cs.LG+quant-ph` (union feed — grep titles for quantum keywords to approximate the intersection) + `curl https://rss.arxiv.org/rss/cs.ET`. CAVEAT: these `rss.arxiv.org` feeds return only the single most recent announcement day, not an arbitrary date range — if the query API stays down for a multi-day gap (e.g. after a weekend), a run using only this fallback will miss any day but the most recent one. Direct `curl https://arxiv.org/abs/<id>` pages worked fine throughout (used to pull abstracts once candidate ids were identified from the RSS titles). Re-verify the query API each run; treat 2+ consecutive occurrences as a forced-heal item per AGENTS.md.
